...
## Bias and Variance of Current Estimators 
...
-$e_{I}(b) = \alpha - (bT)^{-q}G_m'(\chi^\alpha_m)\chi^\alpha_m \overline{B}$


-$e_{II}(b) = G_{m, \delta^2}(\chi^\alpha_m) + (bT)^{-q}G'_{m,\delta^2}(\chi^\alpha_m)\chi^\alpha_m \overline{B} + \frac{\delta^2}{2}G'_{(m+2), \delta^2}(\chi^\alpha_m)\chi^\alpha_m c_2 b$

-$\overline{B} = tr\{B\Sigma^{-1}\}/p$


-$B=-g_q\sum_{h=\infty}^\infty |h|^qEu_tu_{t-h}'$
...
$$
b = \begin{cases}
\left [\frac{2qG'_{m, \delta^2}(\chi^\alpha_m)|\overline{B}|)}{\delta^2 G'_{m+2, \delta^2} (\chi^\alpha_m)c_2} \right ]^{1/(q+1)} T^{-q/(q+1)}& \text{ if } \overline{B}>0 \\ 
\left[\frac{G'_m(\chi^\alpha_m)\chi^\alpha_m |\overline{B}|}{(\tau -1)\alpha)} \right ]^{1/q}\frac{1}{T} & \text{ if } \overline{B}\leq 0 
\end{cases}
$$
...
<br>
<br>
Optimal bandwidth for given set up used. REDO bandwidth [0, 0.5]
