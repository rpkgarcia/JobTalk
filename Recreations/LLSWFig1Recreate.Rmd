---
title: "Attempt2RecreateFig1ofLLS"
...
# Recreation Attempt 2

I already tried to recreate this graph before and made an extensive attempt.  It didn't work for some reason, so I am going to try again.  I am trying "from scratch" but I am using a lot of what I already did in the previous paper.  
...
rho = 0.7
w1 = 2*rho/(1- rho^2)
big_T = 10000    # number of values in data set
...
W = matrix(0, nrow = length(try_b_for_lugsail), ncol = nrow(all_autocovariances))

for(i in 1:length(try_b_for_lugsail)){
  b = try_b_for_lugsail[i]
  M = b*nrow(sim_data)
  new_weights = sapply(0:(M)/(M+1), bartlett)
  W[i, 1:length(new_weights)] = new_weights
}
rownames(W) = paste("b=", round(try_b_for_lugsail, 3), sep = "")
...
rownames(all_estimated_covaraince) = paste("b=", round(try_b_for_lugsail, 3))
...
# for(b in 1:length(try_b)){
#   num = all_estimated_covaraince_lugsail[1:length(try_b), i] - the_lugsail_parameters$c[i]*      all_estimated_covaraince_lugsail[(1:length(try_b) + length(try_b)), i]
#   lugsail_cov[1:length(try_b),i] = num/(1-the_lugsail_parameters$c[i])
# }
num = all_estimated_covaraince_lugsail[1:length(try_b), ] - the_lugsail_parameters$c* all_estimated_covaraince_lugsail[(1:length(try_b) + length(try_b)), ]
lugsail_cov = num/(1-the_lugsail_parameters$c)

lugsail_cov <- function(the_lugsail_parameters){

  if(the_lugsail_parameters$method == "zero"){
        num = all_estimated_covaraince_lugsail[1:length(try_b), ] - the_lugsail_parameters$c* all_estimated_covaraince_lugsail[(1:length(try_b) + length(try_b)), ]
    lugsail_cov = num/(1-the_lugsail_parameters$c)
  } else if(the_lugsail_parameters$method == "adaptive"){
    lugsail_cov = all_estimated_covaraince
    for(b in 1:length(try_b)){
      num = all_estimated_covaraince_lugsail[b, ] - the_lugsail_parameters$c[b]*      all_estimated_covaraince_lugsail[(b + length(try_b)), ]
      lugsail_cov[b,] = num/(1-the_lugsail_parameters$c[b])
    }
  }
...
the_lugsail_parameters <- lugsail_parameters("adaptive", b = try_b)
the_lugsail_cov <- lugsail_cov(the_lugsail_parameters)

results_lugsail = get_size_power(the_means, sim_data, the_lugsail_cov)
```
